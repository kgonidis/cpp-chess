...
namespace engine
{
    ...
    const AttackBoards &AttackBoards::Init()
    {
        ...
        {
            attack_boards.pawn[ESide::WHITE][square] = mask_pawn_attacks(ESide::WHITE, square);
            attack_boards.pawn[ESide::BLACK][square] = mask_pawn_attacks(ESide::BLACK, square);
        ...
        for (int square = 0; square < 64; square++)
        {
            ...
            attack_boards.masks[EAttack::BISHOP][square] = mask_bishop_attacks(square);
            attack_boards.masks[EAttack::ROOK][square] = mask_rook_attacks(square);

            Bitboard masks[] = {attack_boards.masks[EAttack::BISHOP][square], attack_boards.masks[EAttack::ROOK][square]};

            for (int mask_type = 0; mask_type < 2; mask_type++)
            {
                Bitboard attack_mask = attack_boards.masks[mask_type][square];
                auto magic_numbers = (mask_type == EAttack::BISHOP) ? BISHOP_MAGIC_NUMBERS : ROOK_MAGIC_NUMBERS;
                auto relevant_bits = (mask_type == EAttack::BISHOP) ? BISHOP_RELEVANT_BITS : ROOK_RELEVANT_BITS;
        ...
        attack_boards.initialized = true;
        return attack_boards;
    }

    const AttackBoards &AttackBoards::Instance()
    {
        if (!attack_boards.initialized)
            return AttackBoards::Init();
        return attack_boards;
    }

    Bitboard AttackBoards::GetBishopAttacks(int square, Bitboard blockers) const
    {
        Bitboard magic_index = blockers & attack_boards.masks[EAttack::BISHOP][square];
        magic_index *= BISHOP_MAGIC_NUMBERS[square];
        magic_index >>= 64 - BISHOP_RELEVANT_BITS[square];
        return attack_boards.bishop[square][magic_index];
    }

    Bitboard AttackBoards::GetRookAttacks(int square, Bitboard blockers) const
    {
        Bitboard magic_index = blockers & attack_boards.masks[EAttack::ROOK][square];
        magic_index *= ROOK_MAGIC_NUMBERS[square];
        magic_index >>= 64 - ROOK_RELEVANT_BITS[square];
        return attack_boards.rook[square][magic_index];
    }

    Bitboard AttackBoards::GetQueenAttacks(int square, Bitboard blockers) const
    {
